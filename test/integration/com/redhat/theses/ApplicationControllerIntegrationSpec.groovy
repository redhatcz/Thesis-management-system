...
class ApplicationControllerIntegrationSpec extends IntegrationSpec {
    ...
    void "Can approve application"() {
        ...
        where:
            ...
            "supervisor1@example.com" | "supervisor1" | 1
    ...
    void "Can approve and save application"() {
        ...
        expect:
            model == null
            controller.response?.redirectUrl == "/thesis/show/" + ThesisId + "/" + controller.params.thesis.title
            controller.flash.message != null
        where:
            username | password | id | ThesisId
            "admin@example.com" | "admin" | 1 | 8
            "supervisor1@example.com" | "supervisor1" | 1 | 9
            "owner1@example.com" | "owner2" | 4 | 10
    }
    ...
    void "Can decline application"() {
        ...
        where:
            ...
            "supervisor2@example.com" | "supervisor2" | 4
            "supervisor1@example.com" | "supervisor1" | 2
    }
    
    void "Can not create two applications for one user for one topic"() {
        given: "first application"
            controller.request.method = "POST"
            controller.params.application = [:]
            controller.params.application.type = Type.DIPLOMA
            controller.params.application.university = University.findByName("First")
            controller.params.application.topic = Topic.findByTitle('Thesis management system')
            controller.params.application.note = "Test"
            controller.params.application.thesis = new Thesis().save(flush: true)
            
            controller.save()
        when: "trying to save second one, with same parameters"
            def secondApplication = new Application(controller.params.application)
        then: "should fail"
            secondApplication.validate() == false
    }
