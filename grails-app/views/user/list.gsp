...
<body>
    <content tag="active-button">list</content>

    <content tag="main-box">
        <h2 class="header">
            <g:message code="user.list.header" />
        </h2>
        <g:if test="${userInstanceList && userInstanceList.size() != 0}">
            <table class="table table-users">
                <tbody>
                    <g:each
                        in="${(0..(Math.ceil(userInstanceList?.size() / 2) - 1))}"
                        var="i">
                        <tr>
                            <g:each in="${((2*i)..(2*i + 1))}" var="j">
                                <td><g:if
                                        test="${j < userInstanceList.size()}">
                                        <div class="user-info">
                                            <richg:avatar
                                                user="${userInstanceList?.get(j)}"
                                                small="true"
                                                class="img-polaroid avatar-mini"
                                                height="36" width="36" />
                                            <div class="user-text">
                                                <h6>
                                                    <g:link action="show"
                                                        id="${userInstanceList?.get(j)?.id}">
                                                        <g:fieldValue
                                                            bean="${userInstanceList?.get(j)}"
                                                            field="fullName" />
                                                    </g:link>
                                                </h6>
                                                <i class="icon-group"></i>
                                                <g:message
                                                    code="role.${Role.getHighest(userInstanceList?.get(j).roles).toString().toLowerCase()}.label" />
                                            </div>
                                        </div>
                                    </g:if></td>
                            </g:each>
        ...
        <g:else>
            <p class="center muted">
                <g:message code="user.no.users.found" />
            </p>
        </g:else>
